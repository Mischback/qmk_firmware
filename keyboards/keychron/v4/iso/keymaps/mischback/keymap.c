 ...
 *
 * ``BASE`` is just a slight modification of the original layer as provided by
 * Keychron's default keymap for the keyboard (originally named ``WIN_BASE``):
 *   - make CapsLock toggle the first function layer
 *
 * As the DIP switch is disabled, the Mac-specific layers were removed.
 *
 * TODO: Check all keycodes of ``_FN1``, evaluate and then remove them, if not
 *       required.
 * TODO: Use ``_FN1`` for the actual main function layer:
 *         - *VI like movement*: ``hjkl``
 *         - *Gamer movement*: ``wasf`` (Yeah, I know... Left WASD long behind)
 */
